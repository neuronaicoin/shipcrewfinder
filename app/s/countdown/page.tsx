...
const clean = (sp: Record<string, string | undefined>) => {
  const d = Math.max(0, Math.min(999, parseInt(sp.d || "0", 10) || 0));
  const r = (sp.r || "SEAFARER").slice(0, 28).toUpperCase();
  const p = sp.p !== undefined ? Math.max(0, Math.min(100, parseInt(sp.p, 10) || 0)) : null;
  return { d, r, p };
};

const cardUrl = (d: number, r: string, p: number | null) =>
  `/api/card/countdown?d=${d}&r=${encodeURIComponent(r)}${p !== null ? `&p=${p}` : ""}`;

export async function generateMetadata({ searchParams }: { searchParams: SP }): Promise<Metadata> {
  const sp = await searchParams;
  const { d, r, p } = clean(sp);
  const title = `⚓ ${d} days to sign-off — ${r} | ShipCrewFinder`;
  const description =
    "Contract countdown by ShipCrewFinder — the maritime career platform built at sea. Track your contract, build your CV, get found by verified companies.";
  const img = `https://shipcrewfinder.com${cardUrl(d, r, p)}`;
  return {
    title,
    description,
    openGraph: {
...
export default async function CountdownSharePage({ searchParams }: { searchParams: SP }) {
  const sp = await searchParams;
  const { d, r, p } = clean(sp);
...
`}</style>
      ...
      <div className="sw">
        ...
        <div className="scard">
          ...
          <img src={cardUrl(d, r, p)} alt={`${d} days to sign-off — ${r} contract countdown`} width={1200} height={630} />
        </div>

        <h1 className="stitle">Counting down to sign-off? <b>Make the next contract find you.</b></h1>
        <p className="ssub">
          ShipCrewFinder is the maritime career platform built at sea — free CV builder, sea time tracker,
          certificate expiry alerts, live crew chat, and verified companies that contact you directly. Zero commission, ever.
        </p>

        <div className="sbtns">
          <Link href="/signup/crew" className="sgold">⚓ Join free — first month on us</Link>
